...
export async function checkProgram(): Promise<void> {
  ...
  if (priceFeedAccount === null) {
    console.log('Creating account', readingPubkey.toBase58(), 'to read from');
    ...
    try {
      ...
      console.log('Error occurred at sendAndConfirmTransaction.', e);
    }
  }
}
...
export async function getPrice(): Promise<void> {
  ...
  const instruction = new TransactionInstruction({
    keys: [
      { pubkey: readingPubkey, isSigner: false, isWritable: true },
 ...
 */
export async function reportPrice(): Promise<string> {
  // const priceFeedAccount = "FmAmfoyPXiA8Vhhe6MZTr3U6rZfEZ1ctEHay1ysqCqcf"
  // const AggregatorPublicKey = new PublicKey(priceFeedAccount)
  ...
  const rawSolPrice = latestPrice.answer.toString();
  console.log('rawSolPrice: ' + rawSolPrice);
  const solPriceInLamports = parseInt(rawSolPrice);
  const solPrice = solPriceInLamports / LAMPORTS_PER_SOL;
  console.log('solPrice: ', String(solPrice));

  //const rawSolPrice = latestPrice.answer;
  //const solPrice = rawSolPrice / LAMPORTS_PER_SOL;
  //console.log("Current price of SOL/USD is: ", latestPrice.answer.toString())
  return String(solPrice); // latestPrice.answer.toString();
}

export async function getSOLPriceViaChainLink(): Promise<string> {
  // Establish connection to the cluster
  console.log('Calling establishConnection();');
  await establishConnection();

  // Determine who pays for the fees
  console.log('Calling establishPayer();');
  await establishPayer();

  // Check if the program has been deployed
  console.log('Calling checkProgram();');
  await checkProgram();
  ...
  console.log('Calling reportPrice();');
  const solPrice: string = await reportPrice();
